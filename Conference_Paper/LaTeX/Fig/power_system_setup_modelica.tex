...
\begin{tikzpicture}
  ...
  \node[block, fill = green,right= of b] (c) {500 kWh Lithium Battery Array};
  \node[block, fill = magenta, left=of a, xshift  = -4 cm] (d) {Solar Panel Inverter};
  \node[block, fill = pink, left=of d] (e) {100 kW Solar Panel Array};
  \node[block,fill = red, above =of a, yshift = 2cm] (q) {Power Grid};
  \node[block,fill = orange] (l) at (-1.625,-6) {4x Level 2 Chargers};
  \node[block,fill = cyan] (n) at (1.625,-6) {1x Level 3 Charger};
  \node[block,fill=red, below left =of a, xshift = 1.3cm] (o) {Building HVAC Loads};
  \node[block,fill = orange, below right= of a, xshift = -1.3cm] (j){Building Plug Loads};
...
%  \node[block,fill = cyan, below of= q] (n) at {V2G Charger};
  ...
  \node[draw,fill=pink,fill opacity=0.5,inner xsep=5mm,inner
	 ysep=3mm,fit=(a)(o)(j),label={50:UCR Office Building}]{};
  \end{scope}
